			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - Real World Quality Exec</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - Real World Quality Exec</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="http://192.168.178.50/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
